...
import xmltodict    # a library that parses xml files to tables
...
# fetch the metadata for the channel's videos without downloading the video
...
for video_dict in playd['entries']:
    ...
    # when subtitles has elements in its value, it means that there is an uploaded subtitle
    # when the value size is 0, then an automated CC exist. and in this case, you can find it generated in several languages
    if 'subtitles' in video_dict.keys():
        if len(video_dict['subtitles'].keys()) > 0:
            # the dictionary will contain different links for different text and xml formats of the transcript.
            ...
            # You can navigate through the 'automatic_captions' key to check which languages are supported and what is the appropriate abbreviation 
...
# save the data to an excel sheet, you can pick different formats by uncommenting any of the next commented lines
...
#now, we extract the actual text of the transcripts from the links we got from the previous step
transcripts = [] #This will be a variable that you can explore inside the IDE beside the outputted transcripts

# we pick only videos with the values "manual" or "automatic" in the CC column. This means there's a transcript available
for video in relevant_info:
    if video[2] != "not available":
        ...
        # parsing the automatic CCs will be different from parsing manual CCs as the former is in xml format
        if video[3] == 'automatic':
            cc = 'en_automatic_' # will be used for naming the output file. It's specifying the language and pointing that the output file is automatically CCed
            data = response.data # extracting the transcript's xml part from the response object
            data = xmltodict.parse(data) # parsing the xml, which will give a table with the start, end, and text for each timestamp 
            ...
            for slot_dict in data['tt']['body']['div']['p']:
                ...
                # sometimes, a slot contains no text, hence, the key woun't exist. So, we are differentiating the cases to avoid 'Key error'
                if '#text' in slot_dict.keys():
                    text = slot_dict['#text']
                else:
                    text = ""     
                transcript.append("".join([start,' --> ', end, '\n', text, '\n\n']))
        # if CC is manual, then we do not need parsing, The file is already in the correct format. we will pass it as is to our 'transcript' variable
        else:
